...
- All fields must be present in the JSON. Use `null` if a value is not found.
- "policyholderName": Extract the full name of the policyholder (or relevant party name).
- "issueDateRaw": Extract the **original text** for the issue/start date (e.g., "4 July 2025").
- "issueDate": Convert issueDateRaw into "DD-MM-YYYY" format.
- "expirationDateRaw": Extract the original expiration/end date (e.g., "3 July 2026").
- "expirationDate": Convert expirationDateRaw into "DD-MM-YYYY" format.
- "confidence": Provide an estimated confidence score (0–100) for extracted fields like name, dates, and amounts.
- "termsAndExclusions": Extract a list of individual points, especially anything excluded from coverage or terms mentioned in the document.

🧠 Advanced Instructions for Monetary Extraction:
- "premiumAmount": Extract the premium (price paid for the policy).
- "deductibles": Extract the deductible amount (what the policyholder must pay out of pocket).
- Retain the exact format (commas, spacing, currency) as shown in the document.
- Do **not** fabricate a currency if not explicitly present.
- Ignore partial/recurring fees or taxes unless clearly the main amount.

💰 Additional Rule for Final Amount Logic:
- If a field like "contractAmount" or total value is later needed:
  → Extract the **main monetary value** in the document (usually labeled as: 
    "Total Amount", "Contract Value", "Amount Payable", etc.).
  → Avoid line items, taxes, or repeated installment values.
  → Use context clues like: 
     - “shall pay”, “is agreed to be”, “total cost is”, “final amount”, “contract amount”.
  → If multiple candidates exist, choose the one most likely to represent the **total agreed amount**.

⚠️ Important Rules:
- No explanations or notes — only valid JSON.
- Use `null` for any value not clearly found.
- Try your best, even if the input text is messy.
